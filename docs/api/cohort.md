...
| `read_logins_file(path: Path) -> list[str]` | One login per line; strip `#` comments; empty lines ignored. |
| `expand_preset(name: str) -> tuple[str, ...]` | Named login lists: `panel`, `ai`, `famous`, `wide`, `full` (see `cohort_presets.py`). |
| `run_cohort_report(..., min_active_days=30, large_cohort=False) -> dict` | Fetches all users, writes `report_kind: "cohort"`, `statistics/report.json`, `analysis/`, cohort visuals. When `large_cohort` is true, skips O(n²) pairwise user correlation and non-scaling plots; still writes associations and per-metric correlation histograms. `since_policy` is metadata: `"union"`, `"intersection"`, or `"explicit"`. |

## Output layout (under `out_dir`)

- `statistics/report.json` — cohort block includes `cohort_users`, optional `since_policy`, `min_active_days`, `large_cohort`, `correlation_distribution` (per-metric summary stats), `cohort_user_summary`, `cohort_zero_commit_users`, optional `cohort_pca` / `cohort_dendrogram_leaves` / `user_user_spearman_smoothed_30d` (omitted in large-cohort mode), `multi_user_topk`, `commits_summary`, `executive_summary`, `mosaic` (paths), etc.
...
- `visualizations/cohort/` — In **full** mode: `user_pca_scatter.png`, `user_dendrogram.png` (if applicable), `user_weekly_heatmap.png`, `user_summary.png`. In **all** successful association runs: `correlation_distribution_{metric}.png` (Spearman ρ across users), plus `analysis/correlation_distribution_{metric}.csv`. Large-cohort mode omits PCA/dendrogram/heatmap user-summary PNGs; keeps distribution outputs.
...
- Large lists and scaling: [large_cohort.md](../large_cohort.md).
